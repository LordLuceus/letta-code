...
export type ChannelConfigFieldType = "text" | "secret" | "select" | "boolean" | "number" | "string-array" | "key-value-map";
...
export type ChannelAccountPatch = ChannelCommonAccountPatch & ChannelPluginAccountPatch & {
    /** Plugin-owned snake_case config accepted from the websocket protocol. */
    config?: ChannelProtocolConfig;
};
export type ChannelConfigPatch = Pick<ChannelCommonAccountPatch, "dmPolicy" | "allowedUsers"> & ChannelPluginAccountPatch & {
    /** Plugin-owned snake_case config accepted from the websocket protocol. */
    config?: ChannelProtocolConfig;
};
...
export type ChannelMessageActionTransport = Pick<ChannelAdapter, "sendMessage" | "downloadAttachment">;
/** Route identity required by MessageChannel action implementations. */
export type ChannelMessageActionRoute = Pick<ChannelRoute, "accountId" | "chatId" | "chatType" | "threadId" | "agentId" | "conversationId">;
export interface ChannelMessageActionContext {
     ...
     */
    formatText: (text: string) => Pick<OutboundChannelMessage, "text" | "parseMode" | "textStyle">;
}
/**
...
export interface ChannelPlugin {
    ...
    createAdapter(account: ChannelAccount): Promise<ChannelAdapter> | ChannelAdapter;
    runSetup?(): Promise<boolean>;
    resolveAccountDisplayName?(account: ChannelAccount): Promise<string | undefined> | string | undefined;
