...
import type { ApprovalCreate, LettaStreamingResponse, Message } from "@letta-ai/letta-client/resources/agents/messages";
...
export interface ModConversationHandle {
    id: string | null;
    fork: (options?: ModConversationForkOptions) => Promise<ModConversationHandle>;
    getHistory: (options?: ModConversationHistoryOptions) => Promise<Message[]>;
    sendMessageStream: (messages: ModConversationMessage[], options?: ModConversationSendMessageOptions, requestOptions?: ModConversationSendMessageRequestOptions) => Promise<AsyncIterable<LettaStreamingResponse>>;
    /**
...
export type ModSourceScope = "legacy_global" | "global" | "project" | "bundled" | "agent";
...
export type ModEventName = "conversation_open" | "conversation_close" | "tool_start" | "tool_end" | "turn_start" | "turn_end" | "compact_start" | "compact_end" | "llm_start" | "llm_end";
export type ModConversationOpenReason = "startup" | "new" | "resume" | "fork" | "reload";
export type ModConversationCloseReason = "quit" | "new" | "resume" | "fork" | "reload";
...
export type ModCompactTrigger = "manual" | "context_window_overflow" | "context_window_limit";
...
export type ModEventHandler<TName extends ModEventName = ModEventName> = (event: ModEventMap[TName], context: ModEventContext) => ModEventResultMap[TName] | Promise<ModEventResultMap[TName]>;
export interface ModEventRegistration<TName extends ModEventName = ModEventName> {
    handler: ModEventHandler<TName>;
    name: TName;
    owner: ModOwner;
}
export interface ModEventEmissionResult<TName extends ModEventName = ModEventName> {
    diagnostics: ModDiagnostic[];
    handlerCount: number;
    name: TName;
    results: Array<NonNullable<ModEventResultMap[TName]>>;
}
export type ModCapabilityKind = "api" | "command" | "event" | "permission" | "provider" | "tool" | "panel" | "status" | "statusline";
...
export type ModDiagnosticPhase = "transpile" | "import" | "activate" | "legacy_extension" | "package_manifest" | "command_override" | "command.run" | "deprecated_api" | "dispose" | "event" | "permission.check" | "permission.isEnabled" | "report" | "stale_handle" | "status.evaluate" | "statusline.render" | "tool.isEnabled" | "tool.run";
...
export type ModToolRunResult = string | ModToolContent[] | {
...
export type ModSecretResolver = (name: string, options?: {
    envFallback?: boolean;
}) => Promise<string | null>;
...
export interface ModToolRegistration {
    ...
    run: (context: ModToolRunContext) => ModToolRunResult | Promise<ModToolRunResult>;
}
...
export interface ModPermissionRegistration {
    ...
    description?: string;
    isEnabled?: (context: ModInvocationContext) => boolean;
    check: (event: ModPermissionCheckEvent, context: ModPermissionCheckContext) => ModPermissionCheckResult | Promise<ModPermissionCheckResult>;
}
